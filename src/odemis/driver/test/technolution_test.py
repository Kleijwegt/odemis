...
class TestAcquisitionServer(unittest.TestCase):
    ...
    def test_assembleCalibrationMetadata(self):
        MAX_NMBR_POINTS = 9000  # Constant maximum number of setpoints
        # TODO MAX_NMBR_POINTS value of 9000 is not working for the entire range of the dwell time, this value
        #  needs to be increased so that the entire range can be tested. Then the multipication of 0.99 via FIX in line
        #  243  can be deleted. The current implementation works up to 99% of the max dwell time, only big dwell
        #  times give a problem.
        FIX = 0.99
        ...
        for test_repetition in range(0, 1000):
            ...
            random_dwell_time = numpy.round(numpy.random.random() * FIX * scanner.dwellTime.range[1], 9)
            ...
            descan_derived_total_scanning_time = descanner.clockPeriod.value * len(
                    calibration_parameters.x_descan_setpoints)
    ...
    def test_checkMegaFieldExists(self):
        ...
        """
        # TODO K.K. call file name add start to make clear that recent file name is checked.
        ...
        # the current time i.e. time.strftime("testing_file-%Y-%m-%d-%H-%M-%S").
        mega_field_id = self.MPPC.filename.value
