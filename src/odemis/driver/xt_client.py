...
class SEM(model.HwComponent):
    ...
    def aperture_index_info(self):
        ...
        :return (dict with a list of length 2): The range of the aperture index for the keyword "range".
        """
        with self._proxy_access:
            self.server._pyroClaimOwnership()
...
class Scanner(model.Emitter):
    ...
    def __init__(self, name, role, parent, hfw_nomag, **kwargs):
        ...
        pattern_stigmator_range = tuple((i, j) for i, j in zip(pattern_stigmator_range_x, pattern_stigmator_range_y))
        ...
        self.apertureIndex = model.IntContinuous(
            ...
            range=aperture_index_info["range"],
            setter=self._setApertureIndex)

        beamlet_index_info = self.parent.beamlet_index_info()
        beamlet_index_range_x = beamlet_index_info["range"]["x-direction"]
        beamlet_index_range_y = beamlet_index_info["range"]["y-direction"]
        beamlet_index_range = tuple((i, j) for i, j in zip(beamlet_index_range_x, beamlet_index_range_y))
    ...
    def _setPitch(self, pitch):
        # TODO K.K. check spelling micrometers
        self.parent.set_pitch(pitch * 1e6)  # Convert from micro meter to meters.
    ...
    def _setPatternStigmator(self, pattern_stigmator_value):
        self.parent.set_secondary_stigmator()
